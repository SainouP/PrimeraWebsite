...
menuBtns.forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (key === 'explorar') {
      ...
      menuInfo.textContent = infoContent[key] || "";
    }
  });
});
// esta funcion permite para que se vaya automaticamente sobre el scroll
menuBtns.forEach(btn => {
  btn.addEventListener('click', () => {
    const key = btn.getAttribute('data-info');
    ...
    if (key === 'explorar') {
      ...
      // Scroll suave SOLO vertical
      const section = document.getElementById(key === 'aprender' ? 'aprender-mas' : 'contacto');
      if (section) {
        section.scrollIntoView({
          behavior: 'smooth',
          block: 'start',   // Controla el scroll vertical
          inline: 'nearest' // Evita movimiento horizontal
        });
      }
